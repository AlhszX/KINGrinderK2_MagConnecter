...
d_mag_base = k2_outter_d + mag_d - 2;
...
d_up_hole = k2_inner_d - 3.5;

d_down_no_thread = 46.7;//实测46.5-46.6
d_down_thread = k2_inner_d + 3.35;


//上部
translate([0, 0, h_up_no_thread+thread_h+h_up_mag_base + 5])
difference(){
    union(){
        //螺纹前无螺纹挡板
        cylinder(d=d_up_no_thread, h=h_up_no_thread, $fn=fn, anchor=TOP);
        
        //上部螺纹
        translate([0,0,-h_up_no_thread])
            threaded_rod(d=d_up_thread, h=thread_h, pitch=thread_pitch, $fa=4, $fs=1, $fn=fn, blunt_start=false, internal=false, anchor=TOP);
        
        //接磁铁部分
        translate([0,0,-(h_up_no_thread+thread_h)]) 
            cylinder(d=d_mag_base, h=h_up_mag_base, $fn=fn, anchor=TOP);
    }
    //中空部分
    cylinder(d=d_up_hole, h=h_up_no_thread+thread_h+h_up_mag_base, $fn=fn, anchor=TOP);
    
    //接粉杯对位部分
    translate([0,0,-(h_up_no_thread+thread_h+h_up_mag_base-0.5)])
        cylinder(d=d_down_no_thread, h=0.5, $fn=fn, anchor=TOP);
    
    //磁铁槽
    translate([0,0,-(h_up_no_thread+thread_h+h_up_mag_base)])
    for (i = [0:(mag_num-1)])
        rotate([0, 0, i*360/mag_num]) 
            translate([k2_outter_d/2+0.75, 0, 0])
                cylinder(d=mag_d, h=mag_h, $fn=fn);
...
difference(){
    union(){
        //接粉杯无螺纹部分
        difference(){
            cylinder(d=d_mag_base, h=thread_h, $fn=fn, anchor=TOP);
            cylinder(d=d_down_no_thread, h=thread_h, $fn=fn, anchor=TOP);
        }
        ...
        difference(){
            cylinder(d=d_mag_base, h=thread_h, $fn=fn, anchor=TOP);
            threaded_rod(d=d_down_thread, h=thread_h, pitch=thread_pitch, $fa=4, $fs=1, $fn=fn, internal=true, blunt_start=false, anchor=TOP);
            cylinder(d=d_down_no_thread, h=thread_h, $fn=fn, anchor=TOP);
        }
    }
    ...
    //倒角
    translate([0, 0, -2*thread_h]) rotate([180,0,0])
        rotate_extrude(convexity=10, $fn=fn)
            translate([d_mag_base/2 -1, -1, 0])
                difference(){
                    square(1);
