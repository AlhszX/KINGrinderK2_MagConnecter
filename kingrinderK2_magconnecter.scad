...
difference(){
    union(){
        //上部螺纹
        translate([0, 0, 0]) 
            threaded_rod(d=thread_d-thread_offset, h=thread_h, pitch=thread_pitch, $fa=4, $fs=1,$fn=fn, anchor=BOTTOM);
...
difference(){
    ...
    translate([0, 0, -4])
        threaded_rod(d=thread_d+thread_offset, h=thread_h+1, pitch=thread_pitch, $fa=4, $fs=1, $fn=fn, internal=true, anchor=TOP);
...
//测试螺距用
/*
translate([0, outter_d/2, 0])
    difference(){
        cylinder(d=outter_d, h=thread_h, $fn=fn, anchor=TOP);
        threaded_rod(d=thread_d+thread_offset, h=thread_h+5, pitch=1.5, $fa=4, $fs=1, $fn=fn, internal=true, anchor=TOP);
        translate([outter_d/2-7, 0, -2]) linear_extrude(2) text( "1.5", size=3);
    }

translate([0, outter_d*1.5 +1, 0])
    difference(){
        cylinder(d=outter_d, h=thread_h, $fn=fn, anchor=TOP);
        threaded_rod(d=thread_d+thread_offset, h=thread_h+5, pitch=1.6, $fa=4, $fs=1, $fn=fn, internal=true, anchor=TOP);
        translate([outter_d/2-7, 0, -2]) linear_extrude(2) text( "1.6", size=3);
    }

translate([outter_d +1, outter_d*1.5 +1, 0])
    difference(){
        cylinder(d=outter_d, h=thread_h, $fn=fn, anchor=TOP);
        threaded_rod(d=thread_d+thread_offset, h=thread_h+5, pitch=1.7, $fa=4, $fs=1, $fn=fn, internal=true, anchor=TOP);
        translate([outter_d/2-7, 0, -2]) linear_extrude(2) text( "1.7", size=3);
    }
    
translate([outter_d +1, outter_d/2, 0])
    difference(){
        cylinder(d=outter_d, h=thread_h, $fn=fn, anchor=TOP);
        threaded_rod(d=thread_d+thread_offset, h=thread_h+5, pitch=1.8, $fa=4, $fs=1, $fn=fn, internal=true, anchor=TOP);
        translate([outter_d/2-7, 0, -2]) linear_extrude(2) text( "1.8", size=3);
    }
*/
